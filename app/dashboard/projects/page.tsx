...
import { 
  Calendar, Users, Edit, Trash2, HardDrive, 
  Loader2, X, CheckSquare, ChevronDown, Send, MessageCircle, Smile, Edit2, ArrowLeft, ExternalLink, Search, AlertCircle, Plus
} from 'lucide-react';
...
export default function ProjectsPage() {
  ...
  // State for Instagram-style + reaction menu
  ...
  const handleAssignMember = async (userId: string) => {
    ...
    } catch (error) { console.error("メンバーの割り当てに失敗しました", error); }
  };
    ...
    <div className="p-8 bg-slate-50 min-h-screen flex flex-col h-screen overflow-hidden relative text-slate-900">
      <div className="flex justify-between items-center mb-6 shrink-0">
        <div>
          ...
          <p className="text-slate-600 mt-1 text-sm font-medium">参加中のプロジェクトとタスクを確認します。</p>
        </div>
      </div>

      <div className="flex gap-6 flex-1 min-h-0">
        
        {/* Left Side: Search and Project List */}
        <div className="w-1/3 flex flex-col min-h-0">
          
          {/* JUST SEARCH BAR */}
          <div className="relative mb-4 shrink-0 w-full">
                ...
                >
                  ...
                  <div className="mt-2">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-[9px] font-black text-slate-400 tracking-[0.2em] uppercase">進捗</span>
        ...
        {selectedProject ? (
            ...
            <div ref={scrollContainerRef} onScroll={checkScroll} className="p-8 flex-1 overflow-y-auto custom-scrollbar">
              ...
              <div className="flex justify-center mb-10">
                <div className="bg-slate-50 p-6 rounded-[2.5rem] border border-slate-100 shadow-inner flex flex-col items-center">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">プロジェクトの完了状況</p>
                  ...
                  {selectedProject.members && selectedProject.members.length > 0 ? (
                    selectedProject.members.map((member: any) => (
                      <div key={member.id} className="flex items-center gap-2 bg-white border border-slate-200 pl-1 pr-3 py-1 rounded-full shadow-sm">
                        <div className="w-6 h-6 rounded-full overflow-hidden border border-slate-100 bg-slate-100">
              ...
              <div className="mb-8">
                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">プロジェクト詳細</h4>
                  ...
                  {selectedProject.tasks && selectedProject.tasks.length > 0 ? (
                    selectedProject.tasks.map((task: any) => (
                      <div key={task.id} className="flex items-center justify-between p-3 rounded-xl border border-slate-100 bg-slate-50 hover:bg-slate-100 transition-colors text-slate-900">
                        ...
                        <span className={`text-[10px] font-black px-2.5 py-1 rounded-md ${task.status === 'DONE' || task.status === '完了' ? 'bg-emerald-100 text-emerald-700' : task.status === 'IN_PROGRESS' || task.status === '進行中' ? 'bg-blue-100 text-blue-700' : 'bg-slate-200 text-slate-600'}`}>{task.status === 'DONE' ? '完了' : task.status}</span>
                      </div>
                    ))
                  ) : (
              ...
              <div className="absolute inset-0 bg-white z-[100] flex flex-col animate-in slide-in-from-right duration-300">
                                ...
                                {isEditingThis ? (
                                  <div className="min-w-[200px]">
                                    ...
                                    <div className="flex justify-end gap-2 mt-2">
                                      <button onClick={() => setEditingMessageId(null)} className="text-[10px] font-bold opacity-60">キャンセル</button>
                                      <button onClick={handleUpdateMessage} className="text-[10px] font-bold text-white bg-indigo-50/20 px-2 py-0.5 rounded">保存</button>
                                    </div>
                                  </div>
                                ) : msg.text}
                              </div>
                ...
                <div className="relative">
                  ...
                  <form onSubmit={handleSendMessage} className="p-4 bg-white border-t border-slate-100 flex gap-3 items-center shrink-0">
                     ...
                     <input type="text" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="メッセージを入力..." className="flex-1 bg-slate-50 border border-slate-200 rounded-2xl px-5 py-3 text-sm font-medium outline-none focus:ring-2 focus:ring-indigo-500 transition-all" />
          ...
          <div className="w-2/3 bg-white rounded-[2rem] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center text-slate-300 font-black tracking-[0.4em] uppercase">
            プロジェクトを選択してください
          </div>
        )}
      </div>
